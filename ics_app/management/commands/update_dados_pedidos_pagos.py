...
from ics_app.models import DadosPedidosPagos

class Command(BaseCommand):
    help = "Importa dados de DadosPedidosPagos do Excel para o modelo DadosPedidosPagos"

    def handle(self, *args, **options):
        # 1) Leia o Excel de DadosPedidosPagos
        df = pd.read_excel(settings.DADOS_PEDIDOS_PAGOS_PATH)
        ...
        DadosPedidosPagos.objects.all().delete()
        objs = []
        for _, row in df.iterrows():
            objs.append(DadosPedidosPagos(
        ...
        DadosPedidosPagos.objects.bulk_create(objs)

        # 5) Mensagem de sucesso
        self.stdout.write(self.style.SUCCESS(
            f"{len(objs)} registros de DadosPedidosPagos importados com sucesso."
        ))